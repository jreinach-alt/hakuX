# #283 residual -- the +-1 low-byte error on Volume texture Y16 / R16B16 (cloud, desktop-side)

Issue: #283. Base: origin/master @ e2f617ed99. Needs device: no. Needs NDK: no.
Files: docs/lanes/cloud-283/** only (analysis + a named hunk; you edit no hw/ file).
psh.c is lane.wbufdepth24's until PR #268 folds, then lane.pshqueue's; vk/texture.c is lane.remote's.
...
## Done when
docs/lanes/cloud-283/NOTES.md names the reading with the pixel counts behind it; if it is (1) or
