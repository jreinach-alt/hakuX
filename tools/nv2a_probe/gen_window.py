...
def render(size, blocks, nv2a_c, hazards=None):
    ...
    A("}")
    A("")
    A("/* The check the probe actually applies to a write. Both the command")
    A(" * handler and the commit point call THIS, so there is one predicate to")
    A(" * reason about rather than a pair that can drift apart.")
    A(" *")
    A(" * PROBE_ALLOW_HAZARDS is the emulator-only build (see probe/Makefile).")
    A(" * It removes the hazard refusal and NOTHING ELSE: the window allow-list")
    A(" * still applies, so even that build cannot write outside a modelled,")
    A(" * write-enabled block. Both configurations are compiled and mutated by")
    A(" * run_tests.sh; an #ifdef nothing builds is not a safety property. */")
    A("static inline bool nv2a_offset_write_allowed(uint32_t off)")
    A("{")
    A("#ifdef PROBE_ALLOW_HAZARDS")
    A("    return nv2a_offset_writable(off);")
    A("#else")
