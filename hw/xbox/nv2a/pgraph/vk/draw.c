 ...
 *
 *   TEX  pgraph_vk_poll_bound_textures, the first thing begin_pre_draw does,
 *        test-and-clears DIRTY_MEMORY_NV2A_TEX for every bound texture and
 *        bumps r->texture_vram_gen when it finds anything. So a gen bump
 *        across a begin_pre_draw window proves the bits were consumed inside
 *        it, and a bit set again by the time it closes is a guest store
 *        during the upload that followed. That is #44's resolved mechanism
 *        exactly: the upload for draw N reads guest memory after the guest
 *        has begun writing iteration N+1 to the same address.
 *
 *        The window is begin_pre_draw and NOT the bind_textures calls inside
 *        it, which is the whole trick and was wrong on the first attempt.
 *        See the comment on the wrapper for why a bracket around the bind
 *        reports ~0 for a reason that has nothing to do with the hardware.
 *
...
struct HakuxVramRaceStats {
    ...
    uint64_t tex_windows;       /* begin_pre_draw windows observed */
    uint64_t tex_uploads;       /* ... in which dirty bits were consumed */
    uint64_t tex_raced;         /* ... and were set again by the end */
...
int hakux_vram_race_snprintf(char *buf, int bufsize)
{
    ...
    return snprintf(buf, bufsize,
                    " Vr:%llu/%llu Tr:%llu/%llu/%llu Xd:%llu",
 ...
 * e->dyn_blend on the recorded-draw path. What it selects varies with the
 * SURFACE FORMAT, which none of those keys otherwise contains -- and
 * `Blend surface` renders every case into ONE surface at one address,
 * changing only NV097_SET_SURFACE_FORMAT, so a key blind to the format would
 * serve the previous case's blend equation. That is the same class of
 * staleness #43 hit through PshState and #48 hit through the surface binding.
 *
 * GATED ON THE DEVICE FEATURE, so that on a part without dualSrcBlend the
 * field is never set, nothing downstream fires, and the value is bit-for-bit
 * what it is today. psh.c stages PSH_PAD_ALPHA_NONE under the same condition,
 * from the same register and the same expression -- one derivation read
...
/*
 * Point the COLOUR half of the blend at the second source output.
 *
 ...
 *
 * Index 1 carries the combiner's alpha unchanged (psh.c copies it after the
 * alpha test and after #43's fold, so it is exactly the value the blend unit
 * would have consumed today), and SRC1_ALPHA reads it. The substitution is
 * therefore a NO-OP in intent: every draw that does not stamp is untouched,
 * and every draw that does stamp gets the same colour it would have got
...
static void create_pipeline(PGRAPHState *pg)
{
    ...
    if (use_eds3_blend) {
        ...
        if (pgraph_vk_reg_r(pg, NV_PGRAPH_BLEND) & NV_PGRAPH_BLEND_EN) {
            ...
            /*
             * #59. The colour half moves to the second source output so it
             * keeps reading the combiner's alpha; the alpha half becomes the
             * stamp. See pad_write_color_factor.
             *
             * On hardware these formats have no alpha channel at all -- the
             * pad bits are STAMPED, not blended -- so ONE/ZERO/ADD is what
             * reproduces the silicon, not a simplification of it.
             */
            if (pgraph_vk_blend_stamps_pad_alpha(effective_blend)) {
                color_blend_attachment.srcColorBlendFactor =
                    pad_write_color_factor(
                        color_blend_attachment.srcColorBlendFactor);
                color_blend_attachment.dstColorBlendFactor =
                    pad_write_color_factor(
                        color_blend_attachment.dstColorBlendFactor);
...
/*
 * #54's texture read window, and the reason it wraps the whole of
 * begin_pre_draw rather than the pgraph_vk_bind_textures calls inside it.
 *
 * The dirty bits for a bound texture are consumed by
 * pgraph_vk_poll_bound_textures, which is the FIRST thing begin_pre_draw
 * does -- four bitmap test-and-clears per draw, by its own comment -- and
 * not by the bind. The guest's write is therefore already consumed and
 * recorded as a texture_vram_gen bump before any bind_textures call is even
 * reached, so a bracket around the bind would have found the gen unchanged
 * on nearly every window, reported a race rate of ~0, and closed #54 on an
 * artefact of where the bracket was placed. Wrapping begin_pre_draw puts the
 * consume and the upload that reads guest memory inside one window.
 *
 * A gen bump across this window therefore means: the guest had written a
 * bound texture, we consumed the evidence, and we then read that memory. If
 * the bits are set AGAIN by the time the window closes, the guest wrote into
 * the range while the upload was reading it.
 *
 * Both directions of error, stated here rather than discovered later:
 *
 *  - UPPER bound on byte overlap. Pages are 4 KiB, so a write to the same
 *    page but a different byte counts as a hit. It also over-counts a bound
 *    unit whose bits were not consumed in this window but were already set;
 *    the poll consumes every bound unit it is not already flagged on, which
 *    bounds but does not eliminate that.
 *  - LOWER bound on #44's version skew. If the guest COMPLETES iteration
 *    N+1's write before the poll clears the bits, the clear consumes that
 *    write, the upload reads a fully written successor, and nothing is set
 *    again. That is #44's `cut == complete` case and it is invisible here.
 *    No barrier can fix reading the wrong version of a completed write, and
 *    no read-side counter can see it either.
 *
...
static void begin_pre_draw(PGRAPHState *pg)
{
    ...
    g_hakux_vram_race.tex_windows++;
    if (r->texture_vram_gen == gen_before) {
        return;
    }
    ...
    for (int i = 0; i < NV2A_MAX_TEXTURES; i++) {
        ...
        /* The binding's own key is the range the upload actually read. */
        if (vram_range_dirty_checked(r, b->key.texture_vram_offset,
    ...
    if (!pg->clearing) {
        ...
        if (r->eds3_blend_supported) {
            ...
            if (!r->dyn_state.valid ||
                ...
                /* #59: see pad_write_color_factor. blend_reg is the
                 * EFFECTIVE register, so it carries the synthetic field. */
                if (pgraph_vk_blend_stamps_pad_alpha(blend_reg)) {
                    cbeq.srcColorBlendFactor =
                        pad_write_color_factor(cbeq.srcColorBlendFactor);
                    cbeq.dstColorBlendFactor =
                        pad_write_color_factor(cbeq.dstColorBlendFactor);
                                ...
                                ReorderWindowEntry *prev)
{
    ...
    if (r->eds3_blend_supported &&
        ...
        /* #59: see pad_write_color_factor. e->dyn_blend is the EFFECTIVE
         * register -- recorded that way deliberately, unlike the draw queue's
         * q->dyn_blend, which is raw because it is restored into pg->regs_ --
         * so it carries the synthetic field. */
 ...
 * re-uploads.
 *
 * OPENING THE GATE IS NOT ENOUGH: THE STAGE'S REGISTER CACHE HAS TO GO TOO.
 * pgraph_vk_bind_textures skips create_texture for a stage whose registers
 * still match tex_reg_cache while its VRAM memo reads "checked clean this
 * frame". Re-setting a texture before drawing with it writes the same values,
 * which sets texture_dirty but moves no gen, and that shortcut takes it. For
 * a stage built from a surface the memo is about guest memory the draw never
 * wrote, so it vouches for nothing. Whether it is current is timing -- it
 * lasts until the next flip -- so on #184 the gate alone fixed three of
 * Clear's collapsing captures, and which of the X1A7 pair stayed stale moved
 * when an instrument changed the timing.
 *
 * texture_dirty is deliberately NOT set. It would make every bind rebuild a
 * stage the current draw does not sample -- for a copy, recopy it -- on every
 * draw of a render pass that leaves a stage bound to its own target. With the
 * cache invalidated, the rebuild waits for the next write of that stage's
 * registers, which is how Clear's display quad re-sets it. A stage sampled
 * while its own memory is written, with no register write in between, is not
 * reached; it is stale on master too.
 */
