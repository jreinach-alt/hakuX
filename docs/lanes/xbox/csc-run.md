...
- **The XBE.** nxdk_pgraph_tests `6743b6a` plus [`csc10.patch`](csc10.patch)
  (tests branch `hakux/csc-yuv10` `64a4fb3`). The patch adds a
  `Color space conversion` suite, registered after `AlphaFuncTests`.
  - XBE sha256 `61b3a462a431…`, ISO `3d93df7c8025…`.
...
| `CSC_BumpS_Off` / `_On`, `CSC_BumpT_Off` / `_On` | 2 | stage 0 uniform raw (R, G, B) = (128, 64, 192); stage 1 a ramp (R = x or y, G = B = 128); the left quad is the ramp at one pixel per texel (the colour table), the right quad the middle half displaced by `BUMPENVMAP` with matrix 0.125 |
...
- **K4:** `BumpS_Off` and `BumpT_Off` displace by the raw prediction, -16 and
  +16 texels, within 1. This checks the offset readout.
