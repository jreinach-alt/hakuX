...
serve_one() {
    ...
    if [ -n "$title" ]; then
        ...
        touch "$LEASE"
        SERIAL="$SERIAL" CAPTURE_LOG="$rdir/logcat.txt" LOGCAT_SPEC="${LOGCAT_SPEC:-hakuX-audio:I hakuX-audiocap:I hakuX-perf:I hakuX-pages:I hakuX:W VALIDATION:W ValidationLayer:W vulkan:W VulkanLoader:W *:S}" \
...
meta["logcat"] = dict(spec=os.environ.get("LOGCAT_SPEC",
                                          "hakuX-unhandled:W hakuX-audiocap:I hakuX-perf:I hakuX-pages:I hakuX:W VALIDATION:W "
