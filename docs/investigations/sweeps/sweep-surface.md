...
10. **The CPU watch ended before the write-back obligation did.** *(FIXED —
    `21cacb354a`; kept because the shape recurs.)* Three sites in this document
    described the parts and none of them met: `surface_access_callback` scanned
    only `r->surfaces` (site table, "guest CPU touches surface VRAM");
    `invalidate_surface` / `shelve_surface` moved a node to `invalid_surfaces` /
    `shelved_surfaces` *keeping* `draw_dirty`, and unregistered its access
    callback on the way; and
    `pgraph_vk_download_surfaces_in_range_if_dirty` scans **all three** lists.

    So a retired surface kept the right to write itself into guest VRAM, while
    nothing was watching that VRAM any more. The guest reused the memory, and
    the eventual write-back landed on top of it. Measured: a 256x256 target
    retired at `0x02814000` put 94,208 bytes over a 256KB bump-map texture
    built at `0x027eb000` — exactly the overlap — so `Bump map` scored 2/40
    instead of 38/40, and every test in it sampled 36% foreign pixels.

    The general rule, which nothing in this subsystem states: **deferring a
    copy into guest memory is a claim on that memory, and the claim has to be
    watched for as long as it is held.** Once the CPU has written a range, VRAM
    is authoritative for it and the retained copy must be dropped, not flushed.
    The same shape is available to any other deferred write-back here —
    `deferred_downloads` entries outlive their recording point too.
