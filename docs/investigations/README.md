...
| [`line-extent-phase.md`](line-extent-phase.md) | #13's blocker said the floor/ceil phase of silicon's line widening is undetermined by the goldens, so a right fix could never be scored exact. It is determined: vertices truncate to 1/16 px, `E = w*(max+min/2)/max` on those, and a pixel is lit iff its centre is in the LOW-OPEN band. 100.0000% of both endpoints on 8,890 fitted cuts and 12,056 never-looked-at ones. The old instrument recorded the run's LENGTH and threw away its POSITION, which is the half that pins the phase |
...
| [`unhandled-methods-inventory.md`](unhandled-methods-inventory.md) | Logging the pgraph methods we silently drop, because QEMU's trace for them never reaches logcat. Named the blit clip rectangle in registers — class 0x19, fixed as #47, six tests to zero — plus `0x09f8` SET_SWATH_WIDTH, `0x16bc` glEdgeFlag, class 0x72 `NV072_BETA_4` and two `NV_IMAGE_BLIT` beta slots, from the test repo's own sources. Three NV097 methods remain unnamed, and the dispatcher cannot return the log the sweep needs |
...
| [`blend-signed-full-oracle.md`](blend-signed-full-oracle.md) | #43's signed-byte rule tested on the full 1,568-test oracle at region level: 448 of 448 signed captures exact, 176,160,768 of 176,160,768 channels, 0 of 224 factor pairs deviating, on a harness whose unsigned control is exact on 440,401,920. Every near-neighbour rival rejected and each golden's layout pinned to one of 24 orders. Relocates the "fifth quad" to `DrawColorAndAlphaStack` and shows it is #50, not #43 |
...
| [`dxt1-saturation-band.md`](dxt1-saturation-band.md) | #6 second half: the DXT1 ordered dither saturates at both ends, so its output lives in {0} u [11,250] u {255} on a 5-bit channel and {0} u [5,252] u {255} on a 6-bit one. Derived by level set at both channel widths, with the decisive half of the evidence coming from *interpolated* palette entries that carry no code -- which is why it is not the "code 1" exception the issue recorded. Also separates the bands mip residual, which the arm showed was the simulation's artefact and not a defect. PRE-REGISTERED PASS, 16 of 16 checks, five captures repaired to bit-exact and zero structural error left in DXT1 |
| [`guest-visible-vblank.md`](guest-visible-vblank.md) | #64 and #65: the first measurement of the timing the *guest* sees rather than the frame rate we deliver. The VBLANK period is right to 0.17% and its p99 to 1%; the adaptive deferral then takes the delivered rate 5.6% below it and never gives it back, one-sidedly. Includes the derivation that failed by 26.3%, why a mean is useless for jitter, and the 0.0 ms in the pacing doc's simple_vblank row that was an uninitialised counter |
| [`vram-race-frequency-is-per-title.md`](vram-race-frequency-is-per-title.md) | #54's retirement soak, answered off disk: the probe had already run 29 times on four titles and both devices, so the retirement condition was met before the holds lifted. The registered 5.45e-05 reproduces on Galleon over 9 runs (5.54e-05, p=0.45) and does NOT transfer -- the rate is a per-title, per-bound-mode quantity spanning four orders of magnitude, and the 5.35e-05 bound the falsifier names is a Galleon bound used as a bar for every title. Also: the #44 skew bound zeroes BOTH race sites |
| [`issue77-driver-ab-is-not-nova-specific.md`](issue77-driver-ab-is-not-nova-specific.md) | #77's driver A/B was never blocked on the Nova, on three counts: it already ran on the Thor (four arms at `c866527e03`), both handhelds ship a byte-identical stock Adreno driver down to the build string, and the remaining obstacle is the attract demo's signal, which no device changes. Also settles the one live owner conditional -- the Thor's USB dialog is gone, so the tap must not be re-raised |
