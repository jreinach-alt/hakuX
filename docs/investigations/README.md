...
| [`line-width-never-reaches-the-rasteriser.md`](line-width-never-reaches-the-rasteriser.md) | Why every line draws one pixel wide whatever the guest asked for. Coverage per capture against the requested width, which is the measurement that made it legible, and why Vulkan's wide lines cannot express the register |
